...
import WorkApplicationsBackButton from "./WorkApplicationsBackButton";
...
export default async function WorkApplicationsPage() {
  ...
  return <AppShell><main className="min-h-screen bg-gradient-to-br from-[#020617] via-[#07121f] to-[#0f172a]"><div className="mx-auto max-w-5xl px-5 py-8 sm:px-6 sm:py-10">
    <WorkApplicationsBackButton />
    <header className="mt-6 rounded-3xl border border-cyan-400/20 bg-cyan-400/[0.05] p-6 sm:p-8"><ClipboardList className="text-cyan-300" /><p className="mt-4 text-[11px] font-semibold uppercase tracking-[0.2em] text-cyan-300">Your work</p><h1 className="mt-1 text-3xl font-black text-white">Paid Mission applications</h1><p className="mt-3 text-sm leading-6 text-slate-400">Track selection decisions and open your private workspace after acceptance.</p><Link href="/work/evidence" className="mt-5 inline-flex items-center gap-2 rounded-xl border border-emerald-400/25 bg-emerald-400/[0.08] px-4 py-2.5 text-sm font-bold text-emerald-200"><BadgeCheck size={16} aria-hidden="true" />View Verified Work</Link></header>
