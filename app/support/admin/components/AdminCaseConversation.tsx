...
export default function AdminCaseConversation({
  supportCase,
  onReplySent,
}: Props) {
  const [
    ...
    useCallback(
      async () => {
        ...
        try {
          const response =
            await fetch(
              `/api/support/admin/cases/${encodeURIComponent(
                supportCase.id
              )}/messages`,
              {
                method: "GET",
  ...
  async function sendReply(
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          data.success
            ? "ASCEND could not send this reply."
            : data.error
        );
      }
  ...
  return (
    <section className="mt-6 overflow-hidden rounded-2xl border border-white/[0.07] bg-white/[0.025]">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Messages between the user and ASCEND Support
            </p>
          </div>
        </div>
      ...
      <div className="max-h-[480px] min-h-[260px] overflow-y-auto p-5">
        {loading &&
        messages.length ===
          0 ? (
          ...
          <div className="flex min-h-[220px] items-center justify-center text-center">
            <div>
              ...
              <p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">
                Send the first support update to begin the case conversation.
              </p>
            </div>
          </div>
        ) : (
      ...
      {error && (
        <div className="mx-5 mb-4 flex items-start gap-3 rounded-xl border border-rose-400/20 bg-rose-400/[0.07] p-4 text-sm text-rose-200">
          <AlertCircle
            size={18}
      ...
      >
        <div className="mb-4">
          ...
          <input
            ...
            maxLength={200}
            className="mt-2 h-11 w-full max-w-sm rounded-xl border border-white/10 bg-slate-950 px-4 text-sm text-white outline-none placeholder:text-slate-600 focus:border-cyan-400/30 disabled:cursor-not-allowed disabled:opacity-50"
          />
        </div>
